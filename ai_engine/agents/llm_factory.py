...
def build_llm(
    ...
    if provider == "anthropic":
        ...
        resolved = model or os.environ.get("CLAUDE_MODEL_PRIMARY", "claude-3-5-sonnet-20241022")
        logger.info("llm_factory provider=anthropic model=%s streaming=%s", resolved, streaming)
    ...
    if provider == "gemini":
        ...
        logger.info("llm_factory provider=gemini model=%s streaming=%s", resolved, streaming)
    ...
    if or_key and _OPENAI_AVAILABLE and ChatOpenAI is not None:
        logger.info("llm_factory provider=openrouter model=%s streaming=%s", or_model, streaming)
        return ChatOpenAI(
            model=or_model,
            temperature=temperature,
            max_tokens=max_tokens,
    ...
    if gem_key:
        try:
            from langchain_google_genai import ChatGoogleGenerativeAI  # type: ignore
            resolved = model or os.environ.get("GEMINI_MODEL", "gemini-2.0-flash")
            logger.info("llm_factory provider=gemini_auto model=%s streaming=%s", resolved, streaming)
            return ChatGoogleGenerativeAI(
                model=resolved,
                temperature=temperature,
                max_output_tokens=max_tokens,
