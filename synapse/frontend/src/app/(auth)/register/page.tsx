...
export default function RegisterPage() {
  ...
  return (
    <div>
      ...
      <button
        type="button"
        onClick={() => GOOGLE_CONFIGURED && handleGoogleRegister()}
        disabled={googleLoading || !GOOGLE_CONFIGURED}
        title={GOOGLE_CONFIGURED ? 'Continue with Google' : 'Google sign-up is not configured on this deployment'}
      ...
      >
        ...
        {GOOGLE_CONFIGURED ? 'Continue with Google' : 'Google sign-up unavailable'}
      </button>
      ...
      <button
        ...
        title={GITHUB_CONFIGURED ? 'Continue with GitHub' : 'GitHub sign-up is not configured on this deployment'}
        className="flex items-center justify-center gap-3 w-full py-3 rounded-xl text-sm font-medium transition-all duration-200
          border border-slate-200 hover:border-slate-300 bg-white hover:bg-slate-50 text-slate-700
          dark:border-white/10 dark:hover:border-white/20 dark:bg-white/5 dark:hover:bg-white/10 dark:text-slate-200
          disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white dark:disabled:hover:bg-white/5
      ...
      >
        ...
        {GITHUB_CONFIGURED ? 'Continue with GitHub' : 'GitHub sign-up unavailable'}
      </button>

      {/* Note when SSO providers are not configured on this deployment */}
      {(!GOOGLE_CONFIGURED || !GITHUB_CONFIGURED) && (
        <p className="text-[11px] text-slate-400 dark:text-slate-500 text-center mt-3 mb-1 leading-relaxed">
          {!GOOGLE_CONFIGURED && !GITHUB_CONFIGURED
            ? 'Single sign-on providers are not configured on this environment. Please use email and password.'
            : !GOOGLE_CONFIGURED
              ? 'Google sign-up is not configured on this environment.'
              : 'GitHub sign-up is not configured on this environment.'}
        </p>
      )}
      <div className="mb-3" />
