...
4. [Supabase Authentication Setup](#supabase-authentication-setup)
5. [Deploying a New Version](#deploying-a-new-version)
6. [Rollback Procedure](#rollback-procedure)
7. [GitHub Actions Secrets](#github-actions-secrets)
8. [Monitoring & Alerts](#monitoring--alerts)
9. [Troubleshooting Runbooks](#troubleshooting-runbooks)
10. [Database Migrations](#database-migrations)
11. [SSL Certificate Renewal](#ssl-certificate-renewal)
...
SYNAPSE uses **Supabase as the primary authentication layer**, with Django JWT tokens issued on successful verification. Follow these steps once per environment.

### 1. Create a Supabase Project

1. Go to [https://supabase.com/dashboard](https://supabase.com/dashboard) → **New Project**
2. Note down:
   - **Project URL** (`SUPABASE_URL`) — e.g. `https://xipdtobbfdvvlqzoevfw.supabase.co`
   - **anon public key** (`NEXT_PUBLIC_SUPABASE_ANON_KEY`)
   - **service_role secret** (`SUPABASE_SERVICE_ROLE_KEY`) ← keep this server-side only
...
|----------|----------|-------|
| Email/Password | ✅ Yes | Enable "Confirm email" for production |
| Google OAuth | Recommended | Set redirect URI: `https://synapse.app/auth/callback` |
| GitHub OAuth | Optional | Set redirect URI: `https://synapse.app/auth/callback` |

### 3. Configure Supabase Redirect URLs

In Supabase Dashboard → **Authentication → URL Configuration**:
```
Site URL:          https://synapse.app
Redirect URLs:     https://synapse.app/auth/callback
                   https://synapse.app/auth/reset-password
```
...
Add the following to your `.env.prod` file on the EC2 server:

```bash
# ── Supabase Authentication ──────────────────────────────────────────
SUPABASE_URL=https://YOUR_PROJECT_REF.supabase.co
SUPABASE_SERVICE_ROLE_KEY=eyJhbGci...     # service_role key (server-side ONLY)
NEXT_PUBLIC_SUPABASE_URL=https://YOUR_PROJECT_REF.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJhbGci... # anon/public key (safe for browser)
```

> **Security:** `SUPABASE_SERVICE_ROLE_KEY` bypasses Row Level Security. **Never** expose it in the browser bundle or commit it to git.
...
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Anon/public key (for frontend build) |

### 6. Verify Token Exchange Endpoint

```bash
# Obtain a Supabase access token (e.g., by signing in via the frontend)
SUPABASE_TOKEN="eyJhbGci..."

# Exchange for Django JWT
curl -X POST https://synapse.app/api/v1/auth/supabase/ \
  -H "Content-Type: application/json" \
  -d "{\"access_token\": \"$SUPABASE_TOKEN\"}"

# Expected: {"access": "...", "refresh": "...", "user": {...}}
```

### 7. Configure Supabase Webhook (Optional)

For real-time user sync (new signups, deletions):

In Supabase Dashboard → **Database → Webhooks → Create new webhook**:
```
Name:    synapse-user-sync
Table:   auth.users
Events:  INSERT, DELETE
URL:     https://synapse.app/api/v1/auth/supabase/webhook/
Headers: x-webhook-secret: <generate with: python -c "import secrets; print(secrets.token_hex(32))">
```

Add the secret to `.env.prod`:
```bash
...
Complete list of all secrets required for the CI/CD pipeline:
...
| `PRODUCTION_URL` | `https://synapse.app` |
| `NEXT_PUBLIC_API_URL` | `https://api.synapse.app` |
