...
def _make_user(email="billing@test.com"):
    ...
    return User.objects.create_user(
        username=username, email=email, password="TestPass123!", first_name="Billing", last_name="Test"
    )
...
class TestHandleSubscriptionUpdated(TestCase):
    ...
    def setUp(self):
        ...
        self.sub  = _make_subscription(self.user, plan="free", stripe_customer_id="cus_upd")
...
class TestHandleInvoicePaid(TestCase):
    ...
    def setUp(self):
        ...
        self.sub  = _make_subscription(self.user, plan="pro", stripe_customer_id="cus_inv")
...
class TestCheckoutView(TestCase):
    ...
    def test_checkout_creates_session(self, mock_stripe_fn):
        ...
        resp = self.client.post("/api/v1/billing/checkout/", {"plan": "pro"}, format="json")
    ...
    def test_checkout_invalid_plan_returns_400(self):
        resp = self.client.post("/api/v1/billing/checkout/", {"plan": "invalid_plan"}, format="json")
...
class TestCancelView(TestCase):
    ...
    def setUp(self):
        ...
        self.sub    = _make_subscription(self.user, plan="pro", stripe_customer_id="cus_cancel")
