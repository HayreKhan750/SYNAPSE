...
class TestInterestFeedFiltering(TestCase):
    ...
    def setUp(self):
        ...
        self.ai_article_1   = _make_article("GPT-5 Released",           "ai",       ["llm", "gpt"], self.src)
        self.ai_article_2   = _make_article("Transformers Explained",   "ai",       ["deep-learning"], self.src)
        self.sec_article    = _make_article("New CVE Discovered",        "security", ["vulnerability"], self.src)
        self.cloud_article  = _make_article("AWS Cost Optimisation",    "cloud",    ["aws", "cost"], self.src)
        self.webdev_article = _make_article("React 19 Released",         "web dev",  ["react", "javascript"], self.src)
    ...
    def test_for_you_filters_by_ai_interest(self):
        ...
        for topic in topics:
            self.assertIn("ai", topic, f"Unexpected topic in personalised feed: {topic}")
    ...
    def test_for_you_excludes_unrelated_topics(self):
        ...
        for topic in topics:
            self.assertNotIn("ai", topic,    f"AI article leaked into Security-only feed: {topic}")
            self.assertNotIn("cloud", topic, f"Cloud article leaked into Security-only feed: {topic}")
    ...
    def test_for_you_without_for_you_param_returns_all(self):
        ...
        for_you_data = resp_for_you.data.get("data", resp_for_you.data.get("results", []))
    ...
    def test_interest_filter_combined_with_topic_param(self):
        ...
        for topic in topics:
            self.assertIn("security", topic, f"Expected only security articles: {topic}")
