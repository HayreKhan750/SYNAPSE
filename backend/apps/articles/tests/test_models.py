...
class ArticleModelTest(TestCase):
    ...
    def test_duplicate_url_rejected(self):
        Article.objects.create(title='Dup 1', url='https://example.com/dup', source=self.source)
        from django.db import IntegrityError
        with self.assertRaises(IntegrityError):
            Article.objects.create(title='Dup 2', url='https://example.com/dup', source=self.source)
