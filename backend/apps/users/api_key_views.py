...
class APIKeyListCreateView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        if len(name) > 100:
            return Response({'success': False, 'error': 'name too long'}, status=status.HTTP_400_BAD_REQUEST)
        if not isinstance(scopes, list):
            return Response({'success': False, 'error': 'scopes must be a list'}, status=status.HTTP_400_BAD_REQUEST)
