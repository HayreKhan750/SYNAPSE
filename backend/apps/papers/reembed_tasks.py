...
def reembed_all_papers(self, batch_size: int = 32) -> dict:
    ...
    papers  = ResearchPaper.objects.filter(abstract__isnull=False).exclude(abstract="").order_by("id")
    total   = papers.count()
    embedded = 0
    skipped  = 0
    ...
    logger.info("reembed_all_papers: complete — embedded=%d skipped=%d", embedded, skipped)
