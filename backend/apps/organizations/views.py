...
from .models import Organization, Membership, OrganizationInvite, OrgRole, OrgPlan, OrgAuditLog
...
from .permissions import IsOrgAdminOrOwner, IsOrgOwner, IsOrgMember
...
class OrganizationListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if owned_count >= max_orgs:
            return Response(
                {'success': False, 'error': f'You can only own up to {max_orgs} organization(s) on the free plan.'},
        ...
        OrgAuditLog.objects.create(
            ...
            action=OrgAuditLog.Action.ORG_CREATED, resource=org.name,
        )
...
class OrganizationDetailView(APIView):
    ...
    def _get_org(self, pk):
        try:
            return Organization.objects.select_related('owner').prefetch_related('memberships__user').get(pk=pk)
        except Organization.DoesNotExist:
            return None
    ...
    def get(self, request, pk):
        ...
        if not org.is_member(request.user):
            return Response({'success': False, 'error': 'You are not a member of this organization.'}, status=403)
        return Response({'success': True, 'data': _org_response(org, request, detail=True)})
    ...
    def patch(self, request, pk):
        ...
        if not org.is_admin_or_owner(request.user):
            return Response({'success': False, 'error': 'Admin or owner role required.'}, status=403)
    ...
    def delete(self, request, pk):
        ...
        if org.owner != request.user:
            return Response({'success': False, 'error': 'Only the owner can delete this organization.'}, status=403)
        logger.warning("org_deleted: user=%s org=%s", request.user.email, org.slug)
        org.delete()
        return Response({'success': True, 'message': 'Organization deleted.'}, status=204)
...
class MemberListView(APIView):
    ...
    def post(self, request, pk):
        ...
        if not org.is_admin_or_owner(request.user):
            return Response({'success': False, 'error': 'Admin or owner role required.'}, status=403)
        if org.is_full:
            return Response({'success': False, 'error': f'Organization is at member limit ({org.max_members}).'}, status=403)
        ...
        if not created:
            if membership.is_active:
                return Response({'success': False, 'error': 'User is already a member.'}, status=400)
        ...
        logger.info("member_added: actor=%s user=%s org=%s role=%s", request.user.email, user.email, org.slug, role)
        return Response({'success': True, 'data': MembershipSerializer(membership).data}, status=201)
...
class MemberDetailView(APIView):
    ...
    def _get_org_and_membership(self, pk, user_id):
        try:
            org = Organization.objects.get(pk=pk)
            membership = Membership.objects.select_related('user').get(organization=org, user_id=user_id, is_active=True)
    ...
    def patch(self, request, pk, user_id):
        ...
        if new_role not in [r[0] for r in OrgRole.choices if r[0] != OrgRole.OWNER]:
            return Response({'success': False, 'error': 'Invalid role. Cannot assign owner role via API.'}, status=400)

        # Owner cannot have their role changed by an admin
        if membership.role == OrgRole.OWNER and org.owner != request.user:
            return Response({'success': False, 'error': 'Cannot change owner role.'}, status=403)
        ...
        logger.info("role_changed: actor=%s user=%s org=%s new_role=%s", request.user.email, membership.user.email, org.slug, new_role)
        return Response({'success': True, 'data': MembershipSerializer(membership).data})
    ...
    def delete(self, request, pk, user_id):
        ...
        if not is_self and not org.is_admin_or_owner(request.user):
            return Response({'success': False, 'error': 'Admin or owner role required to remove others.'}, status=403)

        # Owner cannot be removed
        if membership.role == OrgRole.OWNER:
            return Response({'success': False, 'error': 'Owner cannot be removed. Transfer ownership first.'}, status=403)
        ...
        logger.info("member_removed: actor=%s user=%s org=%s", request.user.email, membership.user.email, org.slug)
...
class InviteListCreateView(APIView):
    ...
    def post(self, request, pk):
        ...
        if target_user and org.is_member(target_user):
            return Response({'success': False, 'error': 'User is already a member of this organization.'}, status=400)
        ...
        if existing and not existing.is_expired:
            return Response({'success': False, 'error': 'A pending invite already exists for this email.'}, status=400)
        ...
        logger.info("invite_sent: actor=%s email=%s org=%s", request.user.email, email, org.slug)
        return Response({'success': True, 'data': InviteSerializer(invite).data}, status=201)
...
class InviteDeleteView(APIView):
    ...
    def delete(self, request, pk, invite_id):
        try:
            org = Organization.objects.get(pk=pk)
            invite = OrganizationInvite.objects.get(pk=invite_id, organization=org, is_accepted=False)
        ...
        logger.info("invite_cancelled: actor=%s invite=%s org=%s", request.user.email, invite_id, org.slug)
...
class InviteAcceptView(APIView):
    ...
    def post(self, request, token):
        try:
            ...
            return Response({'success': False, 'error': 'Invalid or already-used invite token.'}, status=404)

        if invite.is_expired:
            return Response({'success': False, 'error': 'This invite has expired.'}, status=410)
        ...
        if org.is_full:
            return Response({'success': False, 'error': 'Organization is at member limit.'}, status=403)
        ...
        logger.info("invite_accepted: user=%s org=%s role=%s", request.user.email, org.slug, invite.role)
        return Response({
            'success': True,
