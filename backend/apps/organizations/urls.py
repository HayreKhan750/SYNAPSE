...
urlpatterns = [
    ...
    path('<uuid:pk>/members/<uuid:user_id>/', MemberDetailView.as_view(), name='org-member-detail'),

    # Invites
    path('<uuid:pk>/invites/', InviteListCreateView.as_view(), name='org-invite-list'),
    path('<uuid:pk>/invites/<uuid:invite_id>/', InviteDeleteView.as_view(), name='org-invite-delete'),

    # Accept invite (token-based, no org ID in URL)
    path('invites/<uuid:token>/accept/', InviteAcceptView.as_view(), name='org-invite-accept'),
