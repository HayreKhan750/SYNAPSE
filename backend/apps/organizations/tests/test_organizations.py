...
from apps.organizations.models import Organization, Membership, OrganizationInvite, OrgRole
...
def _make_org(owner, name='Acme Corp'):
    ...
    Membership.objects.create(organization=org, user=owner, role=OrgRole.OWNER, is_active=True)
    return org
...
class OrganizationModelTests(TestCase):
    ...
    def test_is_member_true_for_active_membership(self):
        Membership.objects.create(organization=self.org, user=self.member_user, role=OrgRole.MEMBER, is_active=True)
        self.assertTrue(self.org.is_member(self.member_user))

    def test_is_member_false_for_inactive_membership(self):
        Membership.objects.create(organization=self.org, user=self.member_user, role=OrgRole.MEMBER, is_active=False)
    ...
    def test_is_admin_or_owner_true_for_admin(self):
        Membership.objects.create(organization=self.org, user=self.member_user, role=OrgRole.ADMIN, is_active=True)
    ...
    def test_is_full_free_plan(self):
        ...
        for i in range(4):  # owner + 4 more = 5
            ...
            Membership.objects.create(organization=self.org, user=u, role=OrgRole.MEMBER, is_active=True)
    ...
    def test_invite_is_expired_false_when_no_expiry(self):
        invite = OrganizationInvite.objects.create(
            organization=self.org, invited_by=self.owner, email='x@x.com', role=OrgRole.MEMBER
        )
...
class OrgListCreateAPITests(TestCase):
    ...
    def test_create_org_auto_assigns_owner_membership(self):
        ...
        self.assertTrue(Membership.objects.filter(organization=org, user=self.owner, role=OrgRole.OWNER).exists())
...
class OrgDetailAPITests(TestCase):
    ...
    def test_owner_can_patch(self):
        resp = self._auth(self.owner).patch(self.url, {'name': 'Updated Name'}, format='json')
        self.assertEqual(resp.status_code, 200)
        self.org.refresh_from_db()
        self.assertEqual(self.org.name, 'Updated Name')

    def test_member_cannot_patch(self):
        resp = self._auth(self.member_user).patch(self.url, {'name': 'Hacked'}, format='json')
        self.assertEqual(resp.status_code, 403)
...
class MemberAPITests(TestCase):
    ...
    def setUp(self):
        ...
        self.org = _make_org(self.owner)
        Membership.objects.create(organization=self.org, user=self.admin, role=OrgRole.ADMIN, is_active=True)
        Membership.objects.create(organization=self.org, user=self.member_user, role=OrgRole.MEMBER, is_active=True)
    ...
    def test_add_member_as_admin(self):
        resp = self._auth(self.admin).post(
            self.list_url, {'user_id': str(self.new_user.pk), 'role': 'member'}, format='json'
        )
        self.assertEqual(resp.status_code, 201)
    ...
    def test_remove_self(self):
        url = reverse('org-member-detail', kwargs={'pk': self.org.pk, 'user_id': self.member_user.pk})
    ...
    def test_owner_cannot_be_removed(self):
        url = reverse('org-member-detail', kwargs={'pk': self.org.pk, 'user_id': self.owner.pk})
    ...
    def test_change_member_role(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.org.memberships.get(user=self.member_user).refresh_from_db()
        self.assertEqual(self.org.memberships.get(user=self.member_user).role, OrgRole.ADMIN)
...
class InviteAPITests(TestCase):
    ...
    def test_owner_can_create_invite(self):
        with self.settings(EMAIL_BACKEND='django.core.mail.backends.locmem.EmailBackend'):
            resp = self._auth(self.owner).post(
                self.invite_url, {'email': 'invitee5@acme.com', 'role': 'member'}, format='json'
            )
        self.assertEqual(resp.status_code, 201)
        self.assertTrue(OrganizationInvite.objects.filter(email='invitee5@acme.com', organization=self.org).exists())
    ...
    def test_cancel_invite(self):
        ...
        del_url = reverse('org-invite-delete', kwargs={'pk': self.org.pk, 'invite_id': invite.pk})
