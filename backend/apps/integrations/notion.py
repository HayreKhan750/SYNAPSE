...
def get_authorization_url(state: str = '') -> str:
    ...
    client_id    = getattr(settings, 'NOTION_CLIENT_ID', os.environ.get('NOTION_CLIENT_ID', ''))
    redirect_uri = getattr(settings, 'NOTION_REDIRECT_URI', os.environ.get('NOTION_REDIRECT_URI', ''))
    params = (
...
def exchange_code_for_token(code: str) -> dict:
    ...
    client_id     = getattr(settings, 'NOTION_CLIENT_ID',     os.environ.get('NOTION_CLIENT_ID', ''))
    client_secret = getattr(settings, 'NOTION_CLIENT_SECRET', os.environ.get('NOTION_CLIENT_SECRET', ''))
    redirect_uri  = getattr(settings, 'NOTION_REDIRECT_URI',  os.environ.get('NOTION_REDIRECT_URI', ''))
...
def import_page_to_rag(access_token: str, page_id: str, user) -> bool:
    ...
    try:
        ...
        node, created = KnowledgeNode.objects.get_or_create(
            name=title[:300], entity_type='concept',
            defaults={
                'description': content[:500],
                'source_ids':  [f'notion:{page_id}'],
                'metadata': {'notion_page_id': page_id, 'source': 'notion', 'user_id': str(user.pk)},
            }
        )
        if not created:
...
def export_report_to_notion(access_token: str, parent_page_id: str, title: str, content: str) -> str | None:
    """
