...
class ZoteroClient:
    ...
    def __init__(self, api_key: str, user_id: str, is_group: bool = False):
        ...
        self._base     = (
            ...
            if is_group else
            f'{ZOTERO_API_BASE}/users/{user_id}'
        )
        self._headers  = {
    ...
    def iter_items(self, since_version: int = 0, limit: int = 100) -> Generator[list[dict], None, None]:
        """
...
def import_library(zotero_client: ZoteroClient, user, since_version: int = 0) -> dict:
    ...
    """
    from apps.core.models import KnowledgeNode, KnowledgeEdge  # noqa: PLC0415
    ...
    for page in zotero_client.iter_items(since_version=since_version):
        for raw_item in page:
            ...
            paper_node, _ = KnowledgeNode.objects.get_or_create(
                name=item_data['title'][:300], entity_type='paper',
                defaults={
    ...
    logger.info("Zotero import: %d imported, %d skipped for user %s", imported, skipped, user.pk)
