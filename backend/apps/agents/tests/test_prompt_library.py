...
class TestPromptTemplateModel:
    ...
    def test_categories(self, db, user):
        for cat in ['research', 'coding', 'writing', 'analysis', 'business', 'creative']:
...
class TestPromptListCreate:
    ...
    def test_create_requires_title_min_length(self, client_auth):
        resp = client_auth.post(self.URL, {
            'title': 'AB', 'content': 'Long enough content here', 'category': 'coding'
        }, format='json')
...
class TestMyPrompts:
    ...
    def test_returns_only_own_prompts(self, client_auth, other_client, user, other_user):
