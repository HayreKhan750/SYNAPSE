...
from apps.automation.signals import trending_spike_signal, _fire_event
...
class DispatchEventTriggerTests(TestCase):
    ...
    def test_fires_matching_workflow(self):
        ...
        mock_exec.delay.assert_called_once_with(str(wf.id), trigger_event={'title': 'AI news'})
    ...
    def test_topic_filter_matches(self):
        wf = _make_event_workflow(self.user, event_type='new_article', topic_filter='python')
        with patch('apps.automation.tasks.execute_workflow') as mock_exec:
            mock_exec.delay = MagicMock()
            result = dispatch_event_trigger('new_article', {'title': 'Python 4.0 released'})
    ...
    def test_new_article_signal_dispatches_event(self):
        ...
        with patch('apps.automation.tasks.dispatch_event_trigger') as mock_dt:
            ...
            source, _ = ArticleSource.objects.get_or_create(
                name='Test HN',
                defaults={'url': 'https://news.ycombinator.com', 'source_type': 'hackernews'},
            )
