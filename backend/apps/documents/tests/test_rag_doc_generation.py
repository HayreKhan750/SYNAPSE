...
def _make_sections():
    return [
        ...
        {"heading": "Analysis", "content": "Analysis content goes here.\n\nMore detail."},
    ]
...
class TestRetrieveRagContextPinnedItems(TestCase):
    ...
    def test_fetches_article_by_id(self):
        ...
        with patch("apps.articles.models.Article.objects.get", return_value=mock_article):
    ...
    def test_fetches_paper_by_id(self):
        ...
        with patch("apps.papers.models.ResearchPaper.objects.get", return_value=mock_paper):
            context_text, sources = DocumentGenerateView._retrieve_rag_context(
    ...
    def test_fetches_repository_by_id(self):
        ...
        mock_repo.summary = ""
        mock_repo.abstract = ""
        mock_repo.description = "State-of-the-art Machine Learning for PyTorch and TensorFlow."
        mock_repo.url = ""
        mock_repo.html_url = "https://github.com/huggingface/transformers"

        repo_id = str(uuid.uuid4())
        with patch("apps.repositories.models.Repository.objects.get", return_value=mock_repo):
            context_text, sources = DocumentGenerateView._retrieve_rag_context(
    ...
    def test_handles_db_not_found_gracefully(self):
        ...
        with patch("apps.articles.models.Article.objects.get", side_effect=Article.DoesNotExist):
            context_text, sources = DocumentGenerateView._retrieve_rag_context(
                prompt="test",
                user=None,
                content_types=[],
    ...
    def test_multiple_pinned_items(self):
        ...
        with patch("apps.articles.models.Article.objects.get", return_value=mock_article), \
             patch("apps.papers.models.ResearchPaper.objects.get", return_value=mock_paper):
            context_text, sources = DocumentGenerateView._retrieve_rag_context(
                prompt="research",
                user=None,
                content_types=[],
...
class TestRetrieveRagContextVectorSearch(TestCase):
    ...
    def test_vector_search_returns_sources(self):
        ...
        mock_docs = [
            self._make_doc("LLM Paper", "Large language models are transformative.", "papers"),
            self._make_doc("ML Article", "Machine learning continues to evolve.", "articles"),
        ]
        mock_retriever = MagicMock()
        mock_retriever.invoke.return_value = mock_docs

        with patch("apps.documents.views.SynapseRetriever", return_value=mock_retriever) as MockRetriever:
            # Import is inside the function so we patch at the right level
            with patch.dict("sys.modules", {}):
                pass

        # Patch at the import location within the function
        with patch("ai_engine.rag.retriever.SynapseRetriever", return_value=mock_retriever):
            context_text, sources = DocumentGenerateView._retrieve_rag_context(
    ...
    def test_vector_search_fails_gracefully(self):
        ...
        with patch("ai_engine.rag.retriever.SynapseRetriever", side_effect=Exception("DB unavailable")):
            context_text, sources = DocumentGenerateView._retrieve_rag_context(
                prompt="test query",
                user=None,
                content_types=[],
...
class TestExpandPromptToSectionsTuple(TestCase):
    ...
    def test_returns_tuple_when_no_llm_key(self):
        ...
        with patch.object(DocumentGenerateView, "_get_llm_keys", return_value=("", "")), \
             patch.object(DocumentGenerateView, "_retrieve_rag_context", return_value=("", [])):
    ...
    def test_returns_tuple_with_rag_sources(self):
        ...
        fake_sources = [
            {"id": "1", "content_type": "paper", "title": "Test Paper", "url": "", "snippet": "..."}
        ]
        with patch.object(DocumentGenerateView, "_get_llm_keys", return_value=("", "")), \
             patch.object(DocumentGenerateView, "_retrieve_rag_context",
    ...
    def test_llm_success_returns_tuple(self):
        ...
        fake_json = json.dumps([
            {"heading": "Overview", "content": "This is an overview paragraph.\n\nMore content here with enough words."},
            {"heading": "Details", "content": "Detailed analysis follows.\n\nSecond paragraph with substance here."},
        ])
        fake_sources = [{"id": "x", "content_type": "article", "title": "Ref", "url": "", "snippet": ""}]
        mock_llm = MagicMock()
        mock_llm.invoke.return_value = MagicMock(content=fake_json)

        with patch.object(DocumentGenerateView, "_get_llm_keys", return_value=("fake-key", "")), \
             patch.object(DocumentGenerateView, "_retrieve_rag_context",
...
class TestGeneratedDocumentSourcesMetadata(TestCase):
    ...
    def test_sources_metadata_can_be_saved(self):
        sources = [
            {"id": "abc", "content_type": "paper", "title": "Test Paper", "url": "", "snippet": "..."}
        ]
        doc = GeneratedDocument.objects.create(
            user=self.user,
            title="Test",
...
class TestGeneratedDocumentSerializerSourcesUsed(TestCase):
    ...
    def test_sources_used_in_serializer_output(self):
        ...
        sources = [{"id": "1", "content_type": "paper", "title": "Test Paper", "url": ""}]
        doc = GeneratedDocument.objects.create(
            user=self.user,
            title="Test",
            doc_type="markdown",
...
class TestDocumentGenerateViewRAG(TestCase):
    ...
    def test_sources_metadata_saved_when_rag_returns_sources(self):
        ...
        fake_sources = [
            {"id": "abc", "content_type": "paper", "title": "RAG Paper", "url": "", "snippet": "..."}
        ]
        fake_sections = _make_sections()

        with patch.object(DocumentGenerateView, "_expand_prompt_to_sections",
                          return_value=(fake_sections, fake_sources)), \
             patch.object(DocumentGenerateView, "_call_tool",
                          side_effect=self._fake_call_tool):
            response = self.client.post("/api/v1/documents/generate/", {
                "doc_type": "markdown",
    ...
    def test_content_types_passed_to_expand_sections(self):
        ...
        def fake_expand(prompt, title, doc_type, user=None, model_override='',
                        content_types=None, source_item_ids=None):
        ...
        with patch.object(DocumentGenerateView, "_expand_prompt_to_sections",
             ...
             patch.object(DocumentGenerateView, "_call_tool",
                          side_effect=self._fake_call_tool):
            self.client.post("/api/v1/documents/generate/", {
                "doc_type": "markdown",
                "title": "Filter Test",
                "prompt": "test",
    ...
    def test_prebuilt_sections_bypass_rag(self):
        ...
        with patch.object(DocumentGenerateView, "_retrieve_rag_context") as mock_rag, \
             patch.object(DocumentGenerateView, "_call_tool",
                          side_effect=self._fake_call_tool):
            self.client.post("/api/v1/documents/generate/", {
                "doc_type": "markdown",
