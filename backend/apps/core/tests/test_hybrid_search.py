...
class SearchResultTest(TestCase):
    ...
    def test_final_score_uses_rerank_when_available(self):
        ...
        r = SearchResult(id='1', content_type='article', title='T', snippet='S',
                         obj=None, rrf_score=0.5, rerank_score=0.9)
...
class RRFMergeTest(TestCase):
    ...
    def _make_result(self, rid, content_type='article'):
        from apps.core.search import SearchResult
        return SearchResult(id=rid, content_type=content_type,
                            title=f'Title {rid}', snippet='...', obj=None)
    ...
    def test_rrf_assigns_higher_score_to_top_ranked(self):
        ...
        semantic = [self._make_result('A'), self._make_result('C'), self._make_result('D')]
    ...
    def test_rrf_formula_correctness(self):
        ...
        from apps.core.search import _rrf_merge, RRF_K
...
class BM25SearchTest(TestCase):
    ...
    def setUp(self):
        self.art = _make_article(title='Transformer Neural Network',
                                  summary='A new transformer-based model for language understanding.')
        self.paper = _make_paper(title='BERT Language Model',
    ...
    def test_bm25_respects_limit(self):
        ...
        for i in range(5):
            _make_article(title=f'Transformer model {i}',
    ...
    def test_bm25_topic_filter(self):
        ...
        from apps.core.search import bm25_search
        _make_article(title='Transformer in robotics',
...
class SemanticSearchResultsTest(TestCase):
    ...
    def setUp(self):
        import numpy as np
        self.art = _make_article(title='Deep Learning', summary='Neural network research.')
    ...
    def test_semantic_skips_article_without_embedding(self):
        ...
        from apps.core.search import semantic_search_results
        no_embed = _make_article(title='No Embedding Article',
                                  summary='This one has no embedding.')
...
class HybridSearchTest(TestCase):
    ...
    def test_hybrid_respects_limit(self):
        ...
        for i in range(6):
            a = _make_article(title=f'Transformer model {i}',
...
class BM25EndpointTest(TestCase):
    ...
    def setUp(self):
        ...
        self.art = _make_article(title='Attention Mechanism',
    ...
    def test_bm25_endpoint_returns_422_for_empty_query(self):
        ...
        response = self.client.post('/api/v1/search/bm25/', {'query': ''}, format='json')
...
class HybridEndpointTest(TestCase):
    ...
    def test_hybrid_endpoint_returns_200_or_503(self):
        ...
        response = self.client.post(
            ...
            {'query': 'attention', 'content_types': ['articles'], 'use_reranker': False},
            format='json',
        )
    ...
    def test_hybrid_endpoint_returns_422_for_empty_query(self):
        ...
        response = self.client.post('/api/v1/search/hybrid/', {'query': ''}, format='json')
        self.assertEqual(response.status_code, 422)
