...
class PublicPapersView(APIView):
    ...
    def get(self, request: Request) -> Response:
        ...
        return Response({'success': True, 'count': total, 'next': offset + limit < total, 'data': data})
...
class PublicAIQueryView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        if len(question) > 2000:
            return Response({'success': False, 'error': 'question too long (max 2000 chars)'}, status=400)
...
class PublicSaveContentView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        try:
            ...
            return Response({
                'success': True,
                'data': {'id': str(bookmark.id) if hasattr(bookmark, 'id') else None, 'created': created},
            }, status=201 if created else 200)
        except Exception as exc:
            logger.warning("Content save failed: %s", exc)
            return Response({'success': False, 'error': 'Failed to save content'}, status=500)
