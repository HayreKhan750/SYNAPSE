...
ANALYTICS_ENABLED  = bool(POSTHOG_API_KEY) and os.environ.get("ANALYTICS_ENABLED", "true") == "true"
...
METRIC_SIGNUPS          = _make_counter("synapse_user_signups_total",         "Total user registrations", ["plan"])
METRIC_LOGINS           = _make_counter("synapse_user_logins_total",          "Total user logins", ["method"])
METRIC_AI_QUERIES       = _make_counter("synapse_ai_queries_total",           "Total AI chat/agent queries", ["type"])
METRIC_DOCS_GENERATED   = _make_counter("synapse_documents_generated_total",  "Total documents generated", ["doc_type"])
METRIC_ARTICLES_SCRAPED = _make_counter("synapse_articles_scraped_total",     "Total articles scraped", ["source"])
METRIC_SEARCHES         = _make_counter("synapse_searches_total",             "Total searches performed", ["search_type"])
METRIC_BOOKMARKS        = _make_counter("synapse_bookmarks_total",            "Total bookmark toggles", ["content_type", "action"])
METRIC_AUTOMATION_RUNS  = _make_counter("synapse_automation_runs_total",      "Total automation workflow runs", ["status"])
METRIC_DRIVE_UPLOADS    = _make_counter("synapse_drive_uploads_total",        "Total Google Drive uploads")
METRIC_S3_UPLOADS       = _make_counter("synapse_s3_uploads_total",           "Total S3 uploads")


# ── Event tracking functions ───────────────────────────────────────────────────

def track_signup(user_id: str, email: str, plan: str = "free", properties: dict = None) -> None:
    """Track a new user registration."""
    METRIC_SIGNUPS.labels(plan=plan).inc()
    ph = _get_posthog()
    if ph:
        ph.identify(str(user_id), {"email": email, "plan": plan})
        ph.capture(str(user_id), "user_signed_up", {
            "plan": plan,
            **(properties or {}),
        })
...
def track_ai_query(user_id: str, query_type: str = "chat", model: str = "", tokens: int = 0) -> None:
    """Track AI chat or agent query."""
    METRIC_AI_QUERIES.labels(type=query_type).inc()
    ph = _get_posthog()
    if ph:
        ph.capture(str(user_id), "ai_query", {
...
def track_document_generated(user_id: str, doc_type: str, title: str = "") -> None:
    ...
    ph = _get_posthog()
    if ph:
        ph.capture(str(user_id), "document_generated", {
            "doc_type": doc_type,
            "title":    title,
        })


def track_search(user_id: str, query: str, search_type: str = "keyword", results: int = 0) -> None:
    """Track search query."""
    METRIC_SEARCHES.labels(search_type=search_type).inc()
    ph = _get_posthog()
    if ph:
        ph.capture(str(user_id), "search_performed", {
...
def track_bookmark(user_id: str, content_type: str, action: str = "add") -> None:
    ...
    ph = _get_posthog()
    if ph:
        ph.capture(str(user_id), "bookmark_toggled", {
...
def track_automation_run(user_id: str, workflow_name: str, status: str = "success") -> None:
    """Track automation workflow execution."""
    METRIC_AUTOMATION_RUNS.labels(status=status).inc()
    ph = _get_posthog()
    if ph:
        ph.capture(str(user_id), "automation_run", {
